...
public class BEDPE_Reader extends FileReader {
    ...
    BEDPE_Reader(String path, AppState state) {
        super(path, state);
    }
     ...
     *
     * @param mpPath shows the mpPath of BEDPE file
    ...
    public int readFile() {
        ...
        try {
            ...
            FileInfo fInfo=new FileInfo(mpPath, BEDPE_Reader.class);
            ...
            while (scanner.hasNextLine()) {
                ...
                if (line.replaceAll("\t", "").replaceAll(" ", "").length() == 0) {
                    ...
                    if (chr1 == null || !chr1Name.equals(val[0])) {
                        chr1Name = val[0];
                        tmpChr = state.getChromosome(chr1Name, fInfo);
                        if (tmpChr == null) {
                            System.out.println("\n1-Adding Chromosome to State: '" + chr1Name + "'");
                            chr1 = new Chromosome(0, chr1Name, fInfo);
                    ...
                    if (chr2 == null || !chr2Name.equals(val[3])) {
                        ...
                        chr2Name = val[3];
                        tmpChr = state.getChromosome( chr2Name,fInfo);
                        if (tmpChr == null) {
                            System.out.println("\n2-Adding Chromosome to State: '" + chr2Name + "'");
                            chr2 = new Chromosome(0, chr2Name, fInfo);
